<template>
  <div class="table-container" v-cloak :key="2">
    <!--  查询条件-->
    <div class="table-header">
      <el-row :gutter="20">
        <el-col :span="4">
          <el-input placeholder="请输入订单单号" v-model="SeachParams.orderSn" @keyup.enter="selectall"></el-input>
        </el-col>
        <el-col :span="4">
    ...
    <div class="table-body">
      ...
      >
        ...
        <el-table-column label="服务单号" show-overflow-tooltip align="center">
        ...
        <el-table-column
         ...
         <el-table-column  label="退款金额" :show-overflow-tooltip="true" align="center">
            <template #default="scope">
           <span >{{scope.row.productRealPrice}}</span>
            </template>
        </el-table-column>
        <el-table-column label="申请状态" :show-overflow-tooltip="true" align="center">
          <template #default="scope">
            <span >{{scope.row.status=='0' ?'待处理':scope.row.status=='1' ?'退货中':scope.row.status=='2' ? '已完成':'已拒绝' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="处理时间" show-overflow-tooltip align="center">
          <template #default="scope">
            <i class="el-icon-timer"  v-if="scope.row.handleTime"></i>
            <span
              style="margin-left: 10px"
              v-if="scope.row.handleTime"
            >{{ this.$moment(scope.row.handleTime).format("YYYY-MM-DD ") }}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      options:[{lable:'待处理',value:'0'},{lable:'退货中',value:'1'},{lable:'已完成',value:'2'},{lable:'已拒绝',value:'3'}]
    };
  },
  components: {
    DeTails
